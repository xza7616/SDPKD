...
from mymodelnew.KD_loss.ContrastiveSeg.lib.loss.loss_helper import FSAuxCELoss, FSAuxRMILoss, FSCELoss
...
class PixelContrastLoss(nn.Module, ABC):
    ...
    def forward(self, feats, labels=None, predict=None):
        ...
        predict = predict.mean(dim=1, keepdim=True)
...
if __name__ == '__main__':
    ...
    label = torch.randn(4,480,640)
    pre = torch.randn(4,41,480,640)
    loss = PixelContrastLoss()
    out = loss(rgb,label,pre)
    print(out)